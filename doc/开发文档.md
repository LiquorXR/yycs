...
## 8. 企业微信「客户联系」对接细节（B 阶段未实现，当前为静态活码过渡）

> 现状：报告页 `wecom.qrcodeUrl` 取静态配置 `WECOM_QRCODE_URL`；`DELIVERY_TASKS`/`WECOM_CONTACTS` 表已建模但无写入；`DELIVERED/ADDED_WECOM` 状态无推进。以下为 B 阶段目标设计。

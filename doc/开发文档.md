...
sequenceDiagram
    ...
    UX->>W: 拉起微信客户端完成支付
    W-->>B: notify 回调
    B->>B: 验签 + AES-GCM 解密 + 幂等 + 推进订单状态（自动解锁状态机）
    B-->>W: SUCCESS
    F->>B: GET /api/orders/{orderNo}/report 轮询
    B-->>F: 锁定预览 + 企微「联系我」活码(携带 state=orderNo)
    F->>UX: 展示预览 + 扫码添加企微客服
    Q-->>B: add_external_contact 事件回调（state 关联订单）
    B->>B: 记录外部联系人，人工测算跟进（完整结果企微交付）
```
...
| 订单模块 | 创建、查询、状态机 | 状态：CREATED → PAID → UNLOCKED → DELIVERED → ADDED_WECOM / CLOSED |
| 支付模块 | 统一下单（H5/Native）、查单 | 微信支付 V3 API |
| 回调模块 | 支付 notify 验签 + 解密 + 幂等 + 推进订单状态 | 事务内保证恰好一次解锁（防重复解锁，PAID→UNLOCKED 状态机保留），返回 SUCCESS/FAIL |
| 交付模块 | 企微活码生成与展示 | 付费后报告页展示企微「联系我」活码，完整结果由人工企微交付；解锁后调用企微生成带 `state(订单号)` 的活码 |
...
遵循 `API接口规范模板.md`：统一响应 `{code, message, data}`、分页、幂等键 `Idempotency-Key`。本期投放侧接口全部公开（免鉴权，见模板 §1.3），依赖风控策略（限流/金额白名单/渠道校验）防护；回调接口（`/api/pay/notify`、`/api/wecom/notify`）按微信/企微协议返回，不遵循统一包装（见模板 §2.10~2.11）。
...
- `POST /api/pay/notify`（微信支付回调：验签 + AES-GCM 解密 → 状态更新 → 返回 `SUCCESS`）
...
```json
{
  ...
  "data": {
    "orderNo": "S20260809001",
    "state": "DELIVERED",
    "report": { "title": "张三 · 八字命盘详批（姻缘预览）", "locked": true, "lockedPreview": [{"title": "正缘画像与桃花旺衰节点", "body": "…"}, {"title": "婚后财运走势与家庭财富规划", "body": "…"}] },
    "wecom": {
...
- 配置 `wxpay` 段：`mchid`、`appid`（预留）、`notify_url`、证书路径

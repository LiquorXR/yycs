...
| 测算页 `/calc` | 填写本人姓名、出生年月日/时辰（表单）+ 免费生成**预览报告**（付费后解锁完整版） | 表单校验；生辰时间换算（公历→农历可选）；预览报告含掩码提示「完整版需付费解锁」；信息提交即落库并生成 `profileId` |
...
- `POST /api/pay/notify`（微信支付回调：验签 + AES-GCM 解密 → 状态更新 → 解锁报告 → 返回 `SUCCESS`）
...
```json
{
  ...
  "data": {
    "orderNo": "S20260809001",
    "state": "DELIVERED",
    "report": { "title": "张三 · 八字命盘详批（姻缘预览）", "score": 0, "rank": "缘浅之合 · 宜加经营", "scoreNote": "…", "analysis": { "label": "命理总评", "text": "…" }, "karma": [{"title": "正缘画像 · 桃花期预测", "body": "…"}], "lockedPreview": [{"title": "正缘画像与桃花旺衰节点", "body": "…"}] },
    "wecom": {

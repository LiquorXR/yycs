...
- 支付方式：收钱吧微信小店·代客下单（H5 短链单路径；快手 WebView 内打开收银台完成支付）
...
| 约束 | 说明 |
|---|---|
| 快手 WebView 非微信环境 | 不可使用微信 JSAPI 支付（需服务号与网页授权），支付以 **微信小店 H5 短链（代客下单）** 打开收银台完成支付 |
| 快手 WebView 可能拦截收银台跳转 | 以真机实测为准；拦截时提示用户右上角浏览器打开，`localStorage.last_orderNo` 支持重进恢复 |
...
flowchart TD
    A[H5 落地页打开] --> B{UA 识别}
    B -->|快手 WebView| C[整页跳转微信小店 H5 短链收银台]
    B -->|系统浏览器| C
    C --> F{跳转成功?}
    F -->|是| G[收银台内完成支付]
    F -->|否/被拦截| H[提示右上角浏览器打开<br/>last_orderNo 重进恢复]
    H --> G
    G --> J[支付结果页]
```

> 注：单 H5 路径，`paymentMethod` 兼容 `auto/h5`（归一到 h5）；微信/支付宝在收银台内选择。未配置微信小店时订单仍可创建，`payType/payUrl` 返回 null（`codeUrl` 恒 null）优雅降级。
...
单 H5 路径：前端整页跳转 H5 短链收银台完成支付，后续同一套推送 + 查单逻辑。未配置微信小店时 `payType/payUrl` 为 null，前端展示待支付空态。
...
| 支付页 `/pay/:orderNo` | H5 短链整页跳转收银台 | 未拉起时提示右上角浏览器打开；`last_orderNo` 支持重进恢复 |
...
| 交付模块 | 企微加好友跳转 | 付费后支付成功页/报告页按钮整页跳转企微「联系我」链接（静态 `WECOM_QRCODE_URL`；动态 `get_contact_way state=订单号` 为 B 阶段），完整结果由人工企微交付 |
...
| 拦截测试 | 快手 WebView 内 H5 短链跳转表现（拦截时右上角浏览器打开 + last_orderNo 重进） |
